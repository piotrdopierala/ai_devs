...
const runTool = async (toolCall) => {
    ...
    try {
        ...
        // Auto-handle rate limits and bans — wait and retry transparently
        if (result.code === -9999 || result.code === -735) {
            const waitSec = result.ban?.seconds_left ?? 5;
            log.warn(`Rate limited / banned — waiting ${waitSec + 2}s before retry...`);
...
export const run = async (query) => {
    ...
    await write(`# Firmware Agent Run Log\n\n- **Date**: ${new Date().toISOString()}\n- **Model**: ${api.model}\n\n## Task\n\n${query}\n\n---\n\n`);
    ...
    for (let step = 1; step <= MAX_STEPS; step++) {
        ...
        for (const tc of toolCalls) {
            ...
            printState(`TOOL: ${tc.name}`, args);
            await confirm(`Execute ${tc.name}(${JSON.stringify(args)})?`);
